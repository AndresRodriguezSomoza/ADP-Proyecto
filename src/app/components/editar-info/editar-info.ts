...
export class EditorInfo implements OnInit {
  ...
  escucharCambios(): void {
    // Actualizar contadores y validaciones en tiempo real
  ...
  cargarImagenes(): void {
    this.imagenQuienesSomos = localStorage.getItem('imagen_quienesSomos');
    this.imagenMision = localStorage.getItem('imagen_mision');
    this.imagenVision = localStorage.getItem('imagen_vision');
  }
  ...
  imagenActivada(seccion: string): boolean {
    const activada = localStorage.getItem(`imagen_${seccion}_activada`);
    return activada ? JSON.parse(activada) : true;
  }
}