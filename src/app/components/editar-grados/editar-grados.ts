...
export class EditarGrados implements OnInit {
  ...
  validarDescripcion(descripcion: string): boolean {
    if (!descripcion || descripcion.trim() === '') {
      this.errorDescripcion = '⚠️ La descripción es opcional, pero recomendada';
      return true; // Es opcional, pero mostramos advertencia
    }
  ...
  // Validar nombre del nuevo grado (para agregar)
  ...
  validarPosicion(): boolean {
    if (!this.posicionSeleccionadaOrden || this.posicionSeleccionadaOrden <= 0) {
      this.mensajeValidacion = '❌ Selecciona una posición válida';
      return false;
    }
    const gradoReferencia = this.grados.find(g => g.orden === this.posicionSeleccionadaOrden);
    if (!gradoReferencia) {
      this.mensajeValidacion = '❌ La posición seleccionada no es válida';
      return false;
    }
    return true;
  }

  // Inicializar grados base
  ...
  verificarAutorizacion(): void {
    ...
    // Validar nombre del nuevo grado
    if (!this.validarNuevoGrado(this.nuevoGradoNombre)) {
      return;
    }
    
    // Validar posición
    if (!this.validarPosicion()) {
      return;
    }
